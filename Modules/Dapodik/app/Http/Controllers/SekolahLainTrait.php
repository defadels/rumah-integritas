...
use Modules\Dapodik\Imports\ImportSekolahLainClass;
...
trait SekolahLainTrait
{
    public function indexSekolahLain(Request $request)
    {
        $data_v['title'] = 'Sekolah - Lain Lain';
        $data_v['title_sub'] = 'Sekolah - Lain Lain';
        array_push($this->breadcrumb, ['title' => 'Sekolah','url' => '/'.config('app.backend') . '/dapodik/sekolah','active' => false]);
        array_push($this->breadcrumb, ['title' => 'Lain Lain','url' => '#','active' => true]);
        $data_v['breadcrumb'] = $this->breadcrumb;
        $data_v['append_src'] = array('s' => $request->get('s'));
        $data_v['s'] = $request->has('s') ? $request['s'] : null;
        $data_v['listdata'] = DapodikSekolahLainModel::Filter($request)->paginate(20);
        $data_v['jks'] = collect(['l','p']);
        return view('dapodik::'.$this->theme.'.sekolah-lain.index')->with($data_v);
    }

    public function importSekolahLain(Request $request)
    {
        $data_v['title'] = 'Sekolah - Lain - Import';
        $data_v['title_sub'] = 'Sekolah - Lain - Import';
        array_push($this->breadcrumb, ['title' => 'Sekolah','url' => '/'.config('app.backend') . '/dapodik/sekolah','active' => false]);
        array_push($this->breadcrumb, ['title' => 'Lain Lain','url' => '/'.config('app.backend') . '/dapodik/sekolah-lain','active' => false]);
        array_push($this->breadcrumb, ['title' => 'Import','url' => '#','active' => true]);
        $data_v['breadcrumb'] = $this->breadcrumb;
        return view('dapodik::'.$this->theme.'.sekolah-lain.import')->with($data_v);
    }

    public function storeSekolahLain(Request $request)
    {
        $messages = [
        ...
        if($validator->fails()){
            return Redirect::route('dapodik.sekolah-lain.import')->withErrors($validator)->withInput();
        }
        $file = $request->file('file_attach');
        /*kosongkan data*/
        DapodikSekolahLainModel::truncate();
        $import = new ImportSekolahLainClass;
        $save = Excel::import($import,$file);
        if($save){
            \App\Helpers\NumesaHelper::log('INFO', 'Import Data Sekolah Lain Lain : ' . $file->getClientOriginalName(), \Auth::user()->id);
            \Session::flash('messages', $import->getRowCount().' data berhasil diimport');
            return Redirect::route('dapodik.sekolah-lain.import')->with('status', 'sekolah-lain-imported');
        }else{
            \Session::flash('messages', 'Data gagal diimport');
            return Redirect::route('dapodik.sekolah-lain.import')->with('status', 'sekolah-lain-failed');
        }
    }

    public function destroySekolahLain(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        $pid = $request->get('pid');
        $row = DapodikSekolahLainModel::where('id', '=', $pid)->first();
        if ($row) {
            $name = $row->nama_sekolah;
            $check = $this->checkExistSekolahLain($pid);
            if ($check) {
                return response()->json(['status' => 'error', 'pid' => $pid, 'judul' => 'Failed!', 'pesan' => 'Data ' . $name . ' masih terkait dengan data lainnya']);
            } else {
                $delete = DapodikSekolahLainModel::where('id', '=', $pid)->delete();
                if ($delete) {
                    \App\Helpers\NumesaHelper::log('CRITICAL', 'Delete Sekolah Lain Lain :' . $name, \Auth::user()->id);
    ...
    public function destroySekolahLainAll(Request $request)
    {
        if (!$request->ajax()) {
            return response('Request Tidak Benar', 400);
        }
        ...
        if($i > 0) {
            \App\Helpers\NumesaHelper::log('CRITICAL', 'Delete ' . $i . ' Sekolah Lain Lain', \Auth::user()->id);
    ...
    public function checkExistSekolahLain($id)
    {
        $exist = FALSE;
