                            ...
                            <div class="row mb-2">
                                <div class="col-lg-8">
                                    <form class="d-flex flex-wrap align-items-center"
                                          action="{{route('dapodik.sekolah-lab')}}">
                                    ...
                                    <div class="text-lg-end">
                                        @can('create sekolah')
                                            <button type="button"
                                                    ...
                                                    onclick="window.location='{{route('dapodik.sekolah-lab.import')}}'">
                                                <i
                            ...
                            <div class="table-responsive">
                                <table id="listdata" class="table table-centered table-nowrap mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <th>NPSN</th>
                                        @php
                                            $fields = collect(['lab komputer','lab bahasa','lab ipa','lab fisika','lab biologi']);
                                            foreach ($fields as $field){
                                                foreach ($kondisi_label as $kondisi){
                                                    echo '<th>'.ucwords($field).' '.ucwords($kondisi).'</th>';
                                                }
                                                echo '<th>'.'Jml '.ucwords($field).'</th>';
                                            }
                                        @endphp
                                        <th>Created At</th>
                                    </tr>
                                    ...
                                    @foreach($listdata as $row)
                                        ...
                                        <tr id="data_{{ $row->id }}">
                                            ...
                                            <td>{{$row->npsn}}</td>
                                            @php
                                            $fields = collect(['lab_komputer','lab_bahasa','lab_ipa','lab_fisika','lab_biologi']);
                                            foreach ($fields as $field){
                                                foreach ($kondisis as $kondisi){
                                                    echo '<td>'.$row->{$field.'_'.$kondisi}.'</td>';
                                                }
                                                echo '<td>'.$row->{'jml_'.$field}.'</td>';
                                            }
                                            @endphp
                                            <td>
...
@section('script')
    ...
    <script src="{{asset('')}}modules/num-dapodik/sekolah-lab/app.js?v=1.5"></script>
@endsection
